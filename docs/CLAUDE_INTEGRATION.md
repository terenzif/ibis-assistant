...
This guide describes how to connect **Claude Desktop** to the **Knowledge Server** using a local bridge.

## 🌉 Why a Bridge?

Claude Desktop currently supports "Stdio" (standard input/output) connections for local MCP servers. However, the Knowledge Server runs as a remote HTTP/SSE (Server-Sent Events) service to allow multiple users to share the same knowledge graph.

To connect them, we use a lightweight **MCP Bridge** that runs locally on your machine, talks "Stdio" to Claude, and forwards messages to the remote Knowledge Server.

---

## 🛠️ Installation & Setup

### 1. Prerequisite: The Knowledge Server
Ensure the Knowledge Server is running (e.g., on a shared dev server like `http://localhost:3030`).
...
The Knowledge Server exposes the following tools to Claude:
