...
func TestIngestCodebase_Delta(t *testing.T) {
	...
	t.Run("Skip Unchanged", func(t *testing.T) {
		...
		for _, sql := range mockDB.ExecuteCalls {
			if strings.Contains(sql, "CREATE") && strings.Contains(sql, "file_chunk") {
	...
	t.Run("Update Chunks", func(t *testing.T) {
		...
		for _, sql := range mockDB.ExecuteCalls {
			...
			if strings.Contains(sql, "CREATE") && strings.Contains(sql, "file_chunk") && strings.Contains(sql, "batch_status='pending'") {
				foundUpdate = true
				break
			}
		}
...
func TestIngestCodebase_Batching(t *testing.T) {
	...
	if insertCount != 1 {
		t.Errorf("Expected 1 batched CREATE, got %d", insertCount)
	}
}
