...
func TestIngestCodebase_Delta(t *testing.T) {
	...
	t.Run("Update Chunks", func(t *testing.T) {
		...
		// Verify CREATE (Upsert) is called for chunks
		foundUpdate := false
		for _, sql := range mockDB.ExecuteCalls {
			// Check for CREATE logic
...
func TestIngestCodebase_Batching(t *testing.T) {
	...
	for _, sql := range mockDB.ExecuteCalls {
		if strings.Contains(sql, "CREATE file_chunk") {
			// Actually, the batch contains multiple CREATE statements.
			// But they are all in one execution call (BEGIN TRANSACTION; ... COMMIT;)
			// So we count execution calls containing CREATE file_chunk
			insertCount++
		}
	}
	...
	if insertCount != 1 {
		t.Errorf("Expected 1 batched transaction, got %d. Calls: %v", insertCount, mockDB.ExecuteCalls)
	}
}
