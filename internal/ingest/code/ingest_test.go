...
func TestIngestCodebase_Delta(t *testing.T) {
	...
	t.Run("Skip Unchanged", func(t *testing.T) {
		...
		// Verify no CREATE calls inside TRANSACTION
		for _, sql := range mockDB.ExecuteCalls {
			if strings.Contains(sql, "CREATE file_chunk") || strings.Contains(sql, "INSERT INTO file_chunk") {
	...
	t.Run("Update Chunks", func(t *testing.T) {
		...
		// Verify CREATE (inside transaction) is called for chunks
		foundUpdate := false
		for _, sql := range mockDB.ExecuteCalls {
			// Check for JSON-style assignment in CREATE/IF-CREATE
			if (strings.Contains(sql, "CREATE file_chunk") || strings.Contains(sql, "INSERT INTO file_chunk")) && strings.Contains(sql, "batch_status='pending'") {
				foundUpdate = true
				break
			}
		}
...
func TestIngestCodebase_Batching(t *testing.T) {
	...
	// We expect 1 batch call containing a TRANSACTION with multiple CREATEs
	insertCount := 0
	for _, sql := range mockDB.ExecuteCalls {
		if strings.Contains(sql, "BEGIN TRANSACTION") && strings.Contains(sql, "CREATE file_chunk") {
			insertCount++
		}
	}
